...
func (m *MemStore) GetPet(petID int32) (*Pet, error) {
	...
	if !ok {
		return nil, APIErrorf(ErrIDNotFound, "No pet exists with id %d", petID)
	}
	pet, ok := petData.(Pet)
	if !ok {
		return nil, APIErrorf(0, "Could not read pet data from store")
	}
	return &pet, nil
}
...
func (m *MemStore) PostPet(pet *Pet) error {
	_, ok := m.store.Load(pet.ID)
	if ok {
		return APIErrorf(ErrIDAlreadyExists, "Pet with id %d already exists", pet.ID)
	}
